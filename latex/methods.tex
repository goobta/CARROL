...
\begin{enumerate}
	...
	\item Merge the images together through [STICHTING METHOD]
...
in 2014 [GOODFELLOW 2014]. GANs were a new framework that trained generative
...
adding auxiliary features to inputted data [MIRZA 2014]. These “conditioning
...
on both the MNIST dataset and the respective class labels. [MIRZA 2014] \\
...
combined the two ideas by using CGANs for image super resolution [CHEN]. In
their report, Chen et al. introduce two methods of introducing new features into
their Super Resolution GAN (SRGAN)[CHEN]:
...
improvements over a vanilla GAN [CHEN]. As per Chen et Al.’s findings, the
...
[LINDER NOREN] from Ledig et Al  was used as a bootstrap due to its success in
photo-realistic super resolution [LEDIG]. The class information of the sample
...
[CHEN]. In this case, it is trivial to see how class information (such as
“person”) would be a trivial feature to add, as all of the samples would share
the same information [CHEN]. However, this application hosts a variety of
...
without fault. As Goodfellow discusses [GOODFELLOW NIPS], GANs tend to have
...
base images was Lukin et al.’s Bicubic Interpolation SR model from 2006 [LUKIN].
The Bicubic Interpolation SR model has seen impressive results in a scaling
factor of 2x and is now considered a baseline SR model. For this project,
OpenCV’s bicubic interpolation implementation was used [OPENCV].
